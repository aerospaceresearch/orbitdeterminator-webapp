...
#    dcc.Link('Switch to normal mode', href='/app.py'),
#    html.Br(),
...
html.Div([''' ''']),
    ...
    #html.Div([''' ''']),
    html.Div(['''
       Select filter(for pre-processing):'''], style={'fontSize': 22,'marginBottom':7,'display': 'inline-block'},id='filter-selection_'),
    html.Div(['''
       Select method for keplerian determination:'''], style={'fontSize': 22,'marginBottom':7,'display': 'inline-block','marginLeft':97},id='method-selection_'),
    html.Div([''' ''']),
...
@app.callback(Output('output-div','children'),
              [Input('file-upload','contents'),
               ...
               Input('my-dropdown2_','value'),
               Input('my-dropdown3_','value')])

def display_file(file_content, file_name, dropdown_value,dropdown_value_2,dropdown_value_3,dropdown_value_2_,dropdown_value_3_):
...
####
    if('{}'.format(dropdown_value_2_)=='(sg)'):
              window = golay_window.window(10.0, data_ini)
              # Apply the Savintzky - Golay filter with window = 31 and polynomial parameter = 6
              data1 = sav_golay.golay(data_ini, 31, 6)
              res1 = data1[:, 1:4] - data_ini[:, 1:4]

    elif('{}'.format(dropdown_value_2_)=='(tma)'):
              data1 = triple_moving_average.generate_filtered_data(data_ini, 3)
              res1 = data1[:, 1:4] - data_ini[:, 1:4]

    elif('{}'.format(dropdown_value_2_)=='(bo)'):
              data1= sav_golay.golay(data_ini, 31, 6)
              data1 = triple_moving_average.generate_filtered_data(data1, 3)
              res1 = data1[:, 1:4] - data_ini[:, 1:4]


    elif('{}'.format(dropdown_value_2_)=='(nf)'):
              data1=data_ini
              res1 = data1[:, 1:4] - data_ini[:, 1:4]
####
...
####
    if (data1.shape[0] > 0):
        data_dict1 = [{'No.':'{:03d}'.format(i+1), 't (s)':data1[i][0], 'x '+'{}'.format(dropdown_value):data1[i][1], 'y '+'{}'.format(dropdown_value):data1[i][2], 'z '+'{}'.format(dropdown_value):data1[i][3]} for i in range(data1.shape[0])]
####
...
####
        ...
        if('{}'.format(dropdown_value_3_)=='(ef)'):
            kep1, res1 = e_fit.determine_kep(data1[:,1:])


        elif('{}'.format(dropdown_value_3_)=='(gm)'):
                # Apply gibbs method
                kep_gibbs1 = gibbsMethod.gibbs_get_kep(data1[:,1:])
                kep_final_gibbs1 = lamberts_kalman.kalman(kep_gibbs1, 0.01 ** 2)
                kep_final_gibbs1 = np.transpose(kep_final_gibbs1)
                kep1 = np.resize(kep_final_gibbs1, ((7, 1)))
             


        elif('{}'.format(dropdown_value_3_)=='(in)'):
                 # Apply the interpolation method
                kep_inter1 = interpolation.main(data1)
                # Apply Kalman filters, estimate of measurement variance R = 0.01 ** 2
                kep_final_inter1 = lamberts_kalman.kalman(kep_inter1, 0.01 ** 2)
                kep1 = np.transpose(kep_final_inter1)
                kep1 = np.resize(kep1, ((7, 1)))
                




        elif('{}'.format(dropdown_value_3_)=='(lk)'):
              # Apply Lambert's solution for the filtered data set
                kep_lamb1 = lamberts_kalman.create_kep(data1)
                # Apply Kalman filters, estimate of measurement variance R = 0.01 ** 2
                kep_final_lamb1 = lamberts_kalman.kalman(kep_lamb1, 0.01 ** 2)
                kep1 = np.transpose(kep_final_lamb1)
                kep1 = np.resize(kep1, ((7, 1)))
...
###
        ...
        xyz_fig1 = tools.make_subplots(rows=2,cols=2,specs=[[{},{}],
                                                       [{},{'is_3d':True}]])
        xyz_fig1.append_trace(xz_org,1,1)
        xyz_fig1.append_trace(xz_fit,1,1)
        xyz_fig1.append_trace(xz_cur,1,1)
        xyz_fig1.append_trace(yz_org,1,2)
        xyz_fig1.append_trace(yz_fit,1,2)
        xyz_fig1.append_trace(yz_cur,1,2)
        xyz_fig1.append_trace(xy_org,2,1)
        xyz_fig1.append_trace(xy_fit,2,1)
        xyz_fig1.append_trace(xy_cur,2,1)
        xyz_fig1.append_trace(xyz_org,2,2)
        xyz_fig1.append_trace(xyz_fit,2,2)
        xyz_fig1.append_trace(xyz_cur,2,2)
        xyz_fig1.append_trace(earth_top,2,2)
        xyz_fig1.append_trace(earth_bottom,2,2)

        xyz_fig1['layout']['xaxis1'].update(title='x '+'{}'.format(dropdown_value))
        xyz_fig1['layout']['yaxis1'].update(title='z '+'{}'.format(dropdown_value), scaleanchor='x')
        xyz_fig1['layout']['xaxis2'].update(title='y '+'{}'.format(dropdown_value), scaleanchor='x')
        xyz_fig1['layout']['yaxis2'].update(title='z '+'{}'.format(dropdown_value), scaleanchor='x2')
        xyz_fig1['layout']['xaxis3'].update(title='x '+'{}'.format(dropdown_value), scaleanchor='x')
        xyz_fig1['layout']['yaxis3'].update(title='y '+'{}'.format(dropdown_value), scaleanchor='x3')

        xyz_fig1['layout']['scene1']['xaxis'].update(showticklabels=True, showspikes=False, title='x '+'{}'.format(dropdown_value))
        xyz_fig1['layout']['scene1']['yaxis'].update(showticklabels=True, showspikes=False, title='y '+'{}'.format(dropdown_value))
        xyz_fig1['layout']['scene1']['zaxis'].update(showticklabels=True, showspikes=False, title='z '+'{}'.format(dropdown_value))

        xyz_fig1['layout'].update(width=1050, height=700, margin={'t':50})
        xyz_fig1['layout']['legend'].update(orientation='h')
...
####
        ...
        t_fig1 = tools.make_subplots(rows=4,cols=1,shared_xaxes=True)
        t_fig1.append_trace(xt_org,1,1)
        t_fig1.append_trace(xt_fit,1,1)
        t_fig1.append_trace(xt_cur,1,1)
        t_fig1.append_trace(yt_org,2,1)
        t_fig1.append_trace(yt_fit,2,1)
        t_fig1.append_trace(yt_cur,2,1)
        t_fig1.append_trace(zt_org,3,1)
        t_fig1.append_trace(zt_fit,3,1)
        t_fig1.append_trace(zt_cur,3,1)
        t_fig1.append_trace(rt_org,4,1)
        t_fig1.append_trace(rt_fit,4,1)
        t_fig1.append_trace(rt_cur,4,1)

        t_fig1['layout']['xaxis1'].update(title='t (s)')
        t_fig1['layout']['yaxis1'].update(title='x '+'{}'.format(dropdown_value))
        t_fig1['layout']['yaxis2'].update(title='y '+'{}'.format(dropdown_value), scaleanchor='y')
        t_fig1['layout']['yaxis3'].update(title='z '+'{}'.format(dropdown_value), scaleanchor='y')
        t_fig1['layout']['yaxis4'].update(title='|r| '+'{}'.format(dropdown_value), scaleanchor='y', scaleratio=100)

        t_fig1['layout'].update(width=1050, height=700, margin={'t':50})
        t_fig1['layout']['legend'].update(orientation='h')
...
####
        res_x = go.Histogram(name='Δx', x=res1[:,0])
        res_y = go.Histogram(name='Δy', x=res1[:,1])
        res_z = go.Histogram(name='Δx', x=res1[:,2])

        res_fig1 = tools.make_subplots(rows=1,cols=3,shared_yaxes=True)
        res_fig1.append_trace(res_x,1,1)
        res_fig1.append_trace(res_y,1,2)
        res_fig1.append_trace(res_z,1,3)

        res_fig1['layout']['yaxis1'].update(title='Frequency')
        res_fig1['layout']['xaxis1'].update(title='Δx '+'{}'.format(dropdown_value))
        res_fig1['layout']['xaxis2'].update(title='Δy '+'{}'.format(dropdown_value))
        res_fig1['layout']['xaxis3'].update(title='Δz '+'{}'.format(dropdown_value))

        res_fig1['layout'].update(margin={'t':50}, showlegend=False)
####        
...
####
        coords_ecef,coord_times = ground_track(kep1,data1[0][0])
        ...
        track_fig1 = go.Figure(data=[track, track_cur])
        track_fig1['layout'].update(height=600, width=1050, margin={'t':50})
        track_fig1['layout']['geo']['lataxis'].update(showgrid=True, dtick=30, gridcolor='#ccc')
        track_fig1['layout']['geo']['lonaxis'].update(showgrid=True, dtick=60, gridcolor='#ccc')
        track_fig1['layout']['legend'].update(orientation='h')
...
####
            html.Details([
                ...
                dt.DataTable(rows=[
                    {'Element':'Semi-major Axis',
                     'Value'            :str(kep1[0][0])+'{}'.format(dropdown_value)},
                    {'Element':'Eccentricity',
                     'Value'            :str(kep1[1][0])},
                    {'Element':'Inclination',
                     'Value'            :str(kep1[2][0])+' °'},
...
###
            html.Details([
                html.Summary('''XYZ Plots'''),
                dcc.Graph(id='xyz-plot1', figure=xyz_fig1)
            ], open=True),
###
...
####
            html.Details([
                html.Summary('''Ground Track'''),
                dcc.Graph(id='track-plot1',figure=track_fig1)
            ], open=True),
####
...
####
            html.Details([
                html.Summary('''Time Plots'''),
                dcc.Graph(id='t-plot1', figure=t_fig1)
            ], open=True),
####            
...
####
            html.Details([
                html.Summary('''Residuals'''),
                dcc.Graph(id='res-plot1', figure=res_fig1),
                dt.DataTable(rows=[
                    {' ':'Maximum','Δx '+'{}'.format(dropdown_value):np.max(res1[:,0]),'Δy '+'{}'.format(dropdown_value):np.max(res1[:,1]),'Δz '+'{}'.format(dropdown_value):np.max(res1[:,2])},
                    {' ':'Minimum','Δx '+'{}'.format(dropdown_value):np.min(res1[:,0]),'Δy '+'{}'.format(dropdown_value):np.min(res1[:,1]),'Δz '+'{}'.format(dropdown_value):np.min(res1[:,2])},
                    {' ':'Average','Δx '+'{}'.format(dropdown_value):np.average(res1[:,0]),'Δy '+'{}'.format(dropdown_value):np.average(res1[:,1]),'Δz '+'{}'.format(dropdown_value):np.average(res1[:,2])},
                    {' ':'Standard Deviation','Δx '+'{}'.format(dropdown_value):np.std(res1[:,0]),'Δy '+'{}'.format(dropdown_value):np.std(res1[:,1]),'Δz '+'{}'.format(dropdown_value):np.std(res1[:,2])}
                ], editable=False)
            ], open=True),
####            
        ]
